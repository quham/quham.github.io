    ...
    <div className={`min-h-screen min-h-dvh ${bgColor} relative overflow-hidden`} data-theme={theme}>
      ...
      <section className="relative min-h-screen min-h-dvh flex items-center justify-center">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 py-20 text-center">
          <ScrollFadeIn>
            <div className={`inline-flex items-center gap-2 ${isLight ? 'bg-gradient-to-r from-cyan-400/20 to-blue-500/20 border-cyan-400/30' : 'bg-gradient-to-r from-cyan-400/20 to-blue-500/20 border-cyan-400/30'} backdrop-blur-sm border rounded-full px-6 py-2 mb-8`}>
              <Sparkles className={`w-5 h-5 ${isLight ? 'text-cyan-600' : 'text-cyan-400'}`} />
              <span className={`${isLight ? 'text-cyan-600' : 'text-cyan-400'} font-medium`}>Coming Soon!</span>
            </div>
            <h1 className={`text-5xl md:text-7xl font-bold ${textColor} mb-6 leading-tight`}>
              ...
              <br />
              Academy
              <span className={`block text-2xl md:text-3xl ${isLight ? 'text-gray-600' : 'text-gray-300'} font-normal mt-4`}>— No Coding Required</span>
            </h1>
          </ScrollFadeIn>
          <ScrollFadeIn className="mt-8">
            <p className={`text-xl md:text-2xl ${isLight ? 'text-gray-600' : 'text-gray-300'} mb-12 max-w-3xl mx-auto leading-relaxed`}>
        ...
        <section className={`py-20 relative min-h-dvh ${isLight ? 'bg-gray-50' : 'bg-navy-900'}`}>
          ...
          <div className="max-w-6xl mx-auto px-4 relative z-10">
            ...
            <div className="grid md:grid-cols-2 gap-12">
              <Card className={`${cardBg} shadow-2xl`}>
                <CardHeader>
                  <CardTitle className={`text-2xl ${isLight ? 'text-green-600' : 'text-green-400'} flex items-center`}>
              ...
              <Card className={`${cardBg} shadow-2xl`}>
                <CardHeader>
                  <CardTitle className={`text-2xl ${isLight ? 'text-red-600' : 'text-red-400'} flex items-center`}>
                    <div className={`w-8 h-8 ${isLight ? 'bg-red-500/20' : 'bg-red-400/20'} rounded-full flex items-center justify-center mr-3`}>
                      <X className="w-5 h-5" />
                    </div>
                    Not for you if
                  </CardTitle>
                ...
                <CardContent className="p-6">
                  <ul className={`space-y-4 ${isLight ? 'text-red-700' : 'text-red-300'}`}>
                    {[
                      ...
                      <li key={text} className="flex items-start group transition-colors">
                        <div className={`w-6 h-6 ${isLight ? 'bg-red-500/20' : 'bg-red-400/20'} rounded-full flex items-center justify-center mr-3 mt-0.5 flex-shrink-0 group-hover:bg-red-400/30 transition-colors`}>
                          <X className={`w-3 h-3 ${isLight ? 'text-red-600' : 'text-red-400'}`} />
                        </div>
                        <span dangerouslySetInnerHTML={{ __html: text }} />
                      </li>
                    ))}
      ...
      <ScrollFadeIn>
        <section className={`py-20 relative overflow-hidden ${isLight ? 'bg-white' : 'bg-navy-900'}`}>
          <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-br from-purple-50 via-blue-50 to-cyan-50' : 'bg-gradient-to-br from-purple-900/20 via-blue-900/20 to-cyan-900/20'}`} />
          <div className="max-w-6xl mx-auto px-4 relative z-10">
            <h2 className={`text-4xl font-bold text-center ${textColor} mb-16`}>
            ...
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 justify-items-center">
              {modules.map((module, index) => (
                <Card
                  key={index}
                  className={`group hover:scale-[1.02] transition-all duration-300 ${cardBg} hover:border-cyan-400/50 shadow-2xl hover:shadow-cyan-500/20 ${
                    index === modules.length - 1 ? 'lg:col-start-2' : ''
                  }`}
                >
                  <CardHeader className="pb-2">
                    <div className="flex items-center gap-2 justify-start">
                      <Badge className={`mb-2 ${isLight ? 'bg-gradient-to-r from-blue-600/20 to-blue-700/20 text-blue-700 border-blue-600/30 hover:from-blue-600/30 hover:to-blue-700/30' : 'bg-gradient-to-r from-cyan-400/20 to-blue-500/20 text-cyan-400 border-cyan-400/30 hover:from-cyan-400/30 hover:to-blue-500/30'}`}>
                    ...
                    <CardTitle className={`text-lg ${isLight ? 'text-gray-800' : 'text-white'} ${isLight ? 'group-hover:text-blue-600' : 'group-hover:text-cyan-400'} transition-colors`}>{module.title}</CardTitle>
                  </CardHeader>
                  <CardContent className="pt-2 px-6 pb-6">
                    <div className="relative">
                                             <p className={`${isLight ? 'text-black group-hover:text-black' : 'text-gray-300 group-hover:text-gray-200'} transition-colors line-clamp-2 group-hover:line-clamp-none`}>
                          {module.description}
                        </p>
                      <div className={`absolute bottom-0 right-0 ${isLight ? 'bg-gradient-to-l from-gray-50 to-transparent' : 'bg-gradient-to-l from-white/5 to-transparent'} w-8 h-6 group-hover:hidden transition-all duration-300`} />
                    </div>
                    {module.unlock && (
                      <div className="mt-4">
                        <p className={isLight ? 'text-blue-700' : 'text-cyan-200'}>
                          <Unlock className={`w-4 h-4 ${isLight ? 'text-blue-600' : 'text-cyan-300'} inline mr-2`} />
                          {module.unlock}
                        </p>
                      </div>
                    )}
            ...
            <div className="mt-12 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 text-center">
              ...
              <div>
                ...
                <div className={`${isLight ? 'text-gray-600' : 'text-gray-300'} mt-2`}>Comprehensive Modules</div>
              </div>
              <div>
                ...
                <div className={`${isLight ? 'text-gray-600' : 'text-gray-300'} mt-2`}>Hours Saved Weekly</div>
              </div>
              <div>
      ...
      <ScrollFadeIn>
        <section className={`py-20 relative ${isLight ? 'bg-gray-100' : 'bg-navy-800'}`}>
          <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-b from-gray-100 via-gray-200 to-gray-100' : 'bg-gradient-to-b from-navy-800 via-navy-800 to-navy-800'}`} />
          <div className="max-w-4xl mx-auto px-4 relative z-10">
            ...
            <Card className={`max-w-3xl mx-auto ${cardBg} shadow-2xl`}>
              <CardContent className="p-8">
                <div className="flex flex-col md:flex-row items-center gap-8">
                  ...
                  <div className="text-center md:text-left">
                    ...
                    <p className={`${isLight ? 'text-gray-600' : 'text-gray-300'} leading-relaxed mt-4`}>
      ...
      <ScrollFadeIn>
        <section className={`py-20 relative ${isLight ? 'bg-white' : 'bg-navy-900'}`}>
          <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50' : 'bg-gradient-to-br from-indigo-900/20 via-purple-900/20 to-pink-900/20'}`} />
          <div className="max-w-4xl mx-auto px-4 relative z-10">
            <h2 className={`text-4xl font-bold text-center ${textColor} mb-16`}>
      ...
      <ScrollFadeIn>
        <section className={`py-16 ${isLight ? 'bg-gradient-to-r from-cyan-100 via-blue-100 to-indigo-100' : 'bg-gradient-to-r from-cyan-900 via-blue-900 to-indigo-900'} relative overflow-hidden`}>
          <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-br from-cyan-400/10 via-transparent to-blue-400/10' : 'bg-gradient-to-br from-cyan-500/10 via-transparent to-blue-500/10'}`} />
          <div className={`absolute top-10 left-10 w-20 h-20 ${isLight ? 'bg-cyan-400/20' : 'bg-cyan-400/20'} rounded-full blur-xl animate-pulse`} />
          <div className={`absolute bottom-10 right-10 w-32 h-32 ${isLight ? 'bg-blue-500/20' : 'bg-blue-500/20'} rounded-full blur-xl animate-pulse delay-1000`} />
          <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
            <h2 className={`text-3xl font-bold ${textColor} mb-8`}>
      ...
      <footer className={`py-8 ${textColor} text-center relative ${isLight ? 'bg-gray-50' : 'bg-navy-900'}`}>
        <div className={`absolute inset-0 ${isLight ? 'bg-gradient-to-t from-gray-50 via-gray-50 to-transparent' : 'bg-gradient-to-t from-navy-900 via-navy-900 to-transparent'}`} />
        <div className="max-w-6xl mx-auto px-4 relative z-10">
          ...
          <p className={`text-sm ${isLight ? 'text-gray-500' : 'text-gray-500'}`}>© 2025 AI Advantage Academy. All rights reserved.</p>
        </div>
      </footer>
          ...
          <div className="relative z-10 w-full max-w-xl">
            <Card className={`${cardBg} shadow-2xl`}>
              <CardHeader className="flex items-center justify-between pb-2">
                <CardTitle className={textColor}>Join the Waitlist</CardTitle>
                <Button variant="ghost" className={`${isLight ? 'text-gray-600 hover:text-gray-800' : 'text-white/70 hover:text-white'} p-2 h-8 w-8`} onClick={() => setIsWaitlistOpen(false)}>
                  <X className="w-4 h-4" />
                </Button>
              </CardHeader>
              <CardContent>
